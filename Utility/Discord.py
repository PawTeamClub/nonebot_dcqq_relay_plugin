...
# Discord -> 发送纯文本消息
async def send(username: str, avatar_url:str, message: str):
...
# Discord -> 发送单文件
async def send(username: str, avatar_url:str, Files: File):
    await GlobalVars.DiscordBotObj.execute_webhook(
            webhook_id= GlobalVars.webhook.id,
            token= GlobalVars.webhook.token,
            username= username or "Unknown User (WTF???)",
...
async def send(username: str, avatar_url:str, Files: list[File]):
    await GlobalVars.DiscordBotObj.execute_webhook(
            webhook_id= GlobalVars.webhook.id,
            token= GlobalVars.webhook.token,
            username= username or "Unknown User (WTF???)",
