...
async def handle_qq_message(bot: OneBotBot, event: OneBotGroupMessageEvent):
    if (not bool(GlobalVars.DiscordBotObj)):
        return;
    if str(event.group_id) != GlobalVars.QQ_GROUP_ID:
        return;
    ...
    for segment in event.message:
        if segment.type == "image":
            await Discord.send(user_name, avatar_url, segment.data['url'])
        # Debug Mode
            ...
            await Discord.send(user_name, avatar_url, str(segment))
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False)) as session:
        async with session.get(file_url) as response:
            if response.status == 200:
                ...
                await Discord.send(user_name, avatar_url, file);
            else:
                # 重下的东西在想吧，有点懒
                errorMessage = f"用户 {user_name} 上传了文件：\n"
                errorMessage += f"但是下载文件失败，请联系管理员重新下载文件，HTTP 状态码：{response.status_code}"
                await GlobalVars.DiscordBotObj.send_to(channel_id=int(GlobalVars.DISCORD_CHANNEL_ID), message=errorMessage)
