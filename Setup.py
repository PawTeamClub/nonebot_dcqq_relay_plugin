...
async def init():
    ...
    # 通过频道id查询webhook列表
    # 如果没有创建那就创建，如果有那就直接用
    webhooks = await GlobalVars.DiscordBotObj.get_channel_webhooks(channel_id=int(GlobalVars.DISCORD_CHANNEL_ID));
    ...
    if bool(webhookTemp): 
        ...
        GlobalVars.webhook = await GlobalVars.DiscordBotObj.create_webhook(channel_id=int(GlobalVars.DISCORD_CHANNEL_ID), name=GlobalVars.BOT_NAME);
