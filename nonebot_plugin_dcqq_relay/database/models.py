...
from tortoise.fields.data import JSONField, DatetimeField, IntField, CharField, TextField, BooleanField

class QueueMapping(Model):
    id                      = IntField(pk=True)                     # 主键
    platform                = CharField(max_length=50)              # 平台, onebot 或 discord
    username                = CharField(max_length=100)             # 用户名
    avatar_url              = CharField(max_length=100, null=True)  # 头像链接
    group_id                = CharField(max_length=100)             # 群号
    message_id              = CharField(max_length=64, index=True)  # 消息 ID，寻找来源用
    msg_type                = CharField(max_length=20)              # 消息类型, message, file, recallMessage
    content                 = TextField()                           # 消息内容
    created_at              = DatetimeField(auto_add_now=True)      # 创建时间
    processed               = BooleanField(default=False)           # 是否已处理

    class Meta:
        table = "queue_mappings"
